...
/* Instructions for coding hidden stops and take profit

- Record all SL/TP values when positions are opened
- Check this record every tick to make sure it is up to date (make sure it does not contain info for positions that has been closed)
- Retrieve the order open price of our positions and add the appropriate SL/TP values to it
- Check this new value (open price + SL or TP) against current market price (Bid or Ask) for every position
- Close position(s) if appropriate conditions are met
...
extern double  VolBasedSLMultipler = 4; // Stop Loss Amount in units of Volatility
...
extern double  VolBasedTPMultipler = 0; // Take Profit Amount in units of Volatility
...
extern bool    IsVolatilityStopLossOn_Hidden = True; 
extern double  VolBasedSLMultipler_Hidden = 2; // Stop Loss Amount in units of Volatility
...
extern bool    IsVolatilityTakeProfitOn_Hidden = True; 
extern double  VolBasedTPMultipler_Hidden = 2; // Take Profit Amount in units of Volatility

extern string  Header5 = "----------Volatility Measurement Settings-----------";
extern int     atr_period = 14;

extern string  Header6 = "----------Max Orders-----------";
...
extern string  Header8 = "----------EA General Settings-----------";
...
int init()
  {
      ...
      // If EA disconnects abruptly and there are open positions from this EA, hidden SL and TP records will be gone.
...
int start()
  {
   ...
   if(IsMaxPositionsReached(MaxPositionsAllowed, MagicNumber, OnJournaling) == False){
      if(EntrySignal(CrossTriggered)==1){ // Open Long Positions
         OrderNumber = OpenPositionMarket(OP_BUY, GetLot(IsSizingOn, Lots, Risk, YenPairAdjustFactor, Stop, P), Stop, Take, MagicNumber, Slippage, OnJournaling, P, IsECNbroker, MaxRetriesPerTick);
         
         // Set Stop Loss value for Hidden SL
         if (UseHiddenStopLoss) SetStopLossHidden(OnJournaling, IsVolatilityStopLossOn_Hidden, FixedStopLoss_Hidden, myATR, VolBasedSLMultipler_Hidden, P, OrderNumber);  
        
         // Set Take Profit value for Hidden TP
         if (UseHiddenTakeProfit) SetTakeProfitHidden(OnJournaling, IsVolatilityTakeProfitOn_Hidden, FixedTakeProfit_Hidden, myATR, VolBasedTPMultipler_Hidden, P, OrderNumber);  
        
        
        }

      if(EntrySignal(CrossTriggered)==2){ // Open Short Positions
         OrderNumber = OpenPositionMarket(OP_SELL, GetLot(IsSizingOn, Lots, Risk, YenPairAdjustFactor, Stop, P), Stop, Take, MagicNumber, Slippage, OnJournaling, P, IsECNbroker, MaxRetriesPerTick);
...
20) HandleTradingEnvironment
21) GetErrorDescription
...
*/
   ...
   for (int x = 0; x < ArrayRange(HiddenSLList,0); x++) { // Looping through all order number in list
      ...
      orderTicketNumber = HiddenSLList[x, 0]; // Loops through all the order numbers in the list
...
*/
   ...
   for (int x = 0; x < ArrayRange(HiddenTPList,0); x++) { // Looping through all order number in list
      ...
      doesOrderExist = False;
      orderTicketNumber = HiddenTPList[x, 0]; // Loops through all the order numbers in the list
