...
Falcon v1.8: 
- Included Volatility Trailing Stops
- Included Hidden Volatility Trailing Stops
...
extern string  Header14="----------EA General Settings-----------";
...
//----------Exit Rules (All Opened Positions)-----------
   ...
   if(CountPosOrders(MagicNumber,OP_BUY)>=1 && ExitSignal(CrossTriggered)==1)
     { // Close Long Positions
      //CloseOrderPosition(OP_BUY, OnJournaling, MagicNumber, Slippage, P, RetryInterval); 

     }
   if(CountPosOrders(MagicNumber,OP_SELL)>=1 && ExitSignal(CrossTriggered)==2)
...
//----------Entry Rules (Market and Pending) -----------
   ...
   if(IsLossLimitBreached(IsLossLimitActivated,LossLimitPercent,OnJournaling,EntrySignal(CrossTriggered))==False)
      if(IsMaxPositionsReached(MaxPositionsAllowed,MagicNumber,OnJournaling)==False)
        {
         if(EntrySignal(CrossTriggered)==1)
           { // Open Long Positions
         ...
         else if(EntrySignal(CrossTriggered)==2)
...
26) UpdateVolTrailingList
27) SetVolTrailingStop
28) ReviewVolTrailingStop
29) UpdateHiddenVolTrailingList
30) SetHiddenVolTrailing
31) TriggerAndReviewHiddenVolTrailing
32) HandleTradingEnvironment
33) GetErrorDescription
